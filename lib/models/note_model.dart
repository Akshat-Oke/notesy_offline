...
@HiveType(typeId: 1)
class Note extends HiveObject with ChangeNotifier {
  /// Instantiates a [Note].
  Note(
      {this.id,
      this.title,
  ...
  void removeLabel(String name, BuildContext? context) async {
    if (this.labels != null && context != null) {
      ...
      Navigator.pushNamed(context, '/note', arguments: {'note': this});
    }
  ...
  ///Called by `deleteReminder`. Deletes local reminder time (sets to zero)
  ...
  String? get strRemindAtDate => remindAt!.millisecondsSinceEpoch == 0
      ? null
      : DateFormat.MMMMd().format(remindAt!);
  String? get strRemindAtHM => remindAt!.millisecondsSinceEpoch == 0
      ? null
      : DateFormat.jm().format(remindAt!);
  ...
  List<Widget>? get labelsHere {
    return this
        .labels
        ?.map((e) => labelToWidget(e, insideEditor: true))
        .toList();
  }

  ///Used in note editor page
  List<Widget>? labelsHereFunction(BuildContext context) {
    return this
        .labels
        ?.map((e) => labelToWidget(e, insideEditor: true, context: context))
        .toList();
  }

  Widget labelToWidget(dynamic labelText,
      {bool insideEditor = false, BuildContext? context}) {
    return GestureDetector(
      onTap: insideEditor
          ? () {
              print("Hi");
              this.removeLabel(labelText.toString(), context);
            }
          : null,
      child: Container(
        child: Row(
          ...
          children: [
            Text(
              ...
              style: const TextStyle(
                  color: const Color(0xCCFAFAFA), fontSize: 13.2),
            ),
  ...
  Map<String, dynamic> toJson() => {
        ...
        'labels': labels,
        'remindAt': (remindAt ?? DateTime.fromMillisecondsSinceEpoch(0))
...
Note? toNote(DocumentSnapshot doc) => doc.exists
    ? Note(
        ...
        remindAt:
            DateTime.fromMillisecondsSinceEpoch(doc.data()?['remindAt'] ?? 0),
        createdAt:
